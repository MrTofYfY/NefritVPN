...
async def sync_user_to_servers(user_uuid, user_path, action="add"):
    ...
    for server in SERVERS:
        if server["is_master"]:
            continue
        tasks.append(notify_server(server["url"], user_uuid, user_path, action))
...
async def check_trial_used(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        cursor = await db.execute("SELECT 1 FROM trial_used WHERE user_id = ?", (user_id,))
        return await cursor.fetchone() is not None


async def mark_trial_used(user_id):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT OR IGNORE INTO trial_used (user_id) VALUES (?)", (user_id,))
        await db.commit()
...
async def activate_key(key, user_id, username):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            ...
            (user_id, username, datetime.now().isoformat(), key)
        )
        await db.commit()
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if user_row:
            await db.execute("UPDATE keys SET expires_at = ? WHERE id = ?", (user_row[0], key_id))
            await db.execute("UPDATE users SET key_id = ? WHERE user_id = ?", (key_id, user_id))
            await db.commit()
...
async def add_days_to_user(user_id, days):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("UPDATE users SET expires_at = ? WHERE user_id = ?", (new_expires, user_id))
        await db.commit()
...
async def cleanup_expired():
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if expired:
            await db.execute("DELETE FROM users WHERE expires_at IS NOT NULL AND expires_at < ?", (now,))
            await db.commit()
...
async def generate_xray_config():
    ...
    clients = [{"id": u[0], "level": 0} for u in users] or [{"id": str(uuid.uuid4()), "level": 0}]
    config = {
...
async def handle_health(request):
    return web.json_response({"status": "ok", "xray": xray_process is not None and xray_process.poll() is None})
...
def format_expiry(expires_at, is_revoked=False):
    ...
    try:
        ...
        return f"{diff} дн." if diff > 0 else "Истёк"
    except:
        return "?"
...
async def cmd_start(msg: types.Message, command: CommandObject, state: FSMContext):
    ...
    if referrer_id and not await check_user_exists(user_id):
        ...
        try:
            await bot.send_message(referrer_id, f"🎉 Пользователь {username} присоединился по вашей ссылке!\n+{REFERRAL_BONUS_DAYS} дней к подписке!")
        except:
            pass
        return
...
async def buy_menu(cb: types.CallbackQuery):
    trial_available = not await check_trial_used(cb.from_user.id)
    await safe_edit(cb.message, "<b>Выберите тариф:</b>", buy_kb(trial_available))
    await cb.answer()
...
async def referral_menu(cb: types.CallbackQuery):
    count, referred_by = await get_referral_stats(cb.from_user.id)
    ref_link = f"https://t.me/{BOT_USERNAME}?start=ref_{cb.from_user.id}"
    
    text = f"<b>Реферальная система</b>\n\n"
    text += f"За каждого друга: <b>+{REFERRAL_BONUS_DAYS} дней</b>\n"
    text += f"Друг получит: <b>{TRIAL_DAYS_REFERRAL} дней</b>\n\n"
    text += f"Приглашено: <b>{count}</b>\n\n"
...
async def successful_payment(msg: types.Message):
    ...
    expires_at = user_info[3] if user_info else None
    
    sub_url = f"{BASE_URL}/sub/{path}"
    link = generate_vless_link(user_uuid, SERVERS[0])
    exp_str = datetime.fromisoformat(expires_at).strftime("%d.%m.%Y %H:%M") if expires_at else "Бессрочно"
...
async def activate(cb: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit(
        ...
        InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="Отмена", callback_data="back")]])
    )
    await cb.answer()
...
async def new_key_menu(cb: types.CallbackQuery, state: FSMContext):
    ...
    await safe_edit(cb.message, "<b>Выберите срок:</b>", days_kb())
    await cb.answer()
...
async def key_info(cb: types.CallbackQuery):
    ...
    text += f"Срок: {days_str}\n"
    text += f"Юзер: @{username if username else '-'}\n"
    text += f"Осталось: {format_expiry(expires_at, is_revoked)}"
